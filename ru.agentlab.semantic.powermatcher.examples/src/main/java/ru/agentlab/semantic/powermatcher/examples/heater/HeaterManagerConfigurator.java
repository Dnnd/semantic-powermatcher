...
@Designate(ocd = HeaterManagerConfigurator.Config.class, factory = true)
public class HeaterManagerConfigurator implements ThingServiceConfigurator {
    ...
    public Dictionary<String, ?> getConfiguration(Thing thing, ConnectionContext context) {
        ...
        try (var res = conn.prepareTupleQuery(query.getQueryString()).evaluate()) {
            for (BindingSet bindings : res) {
                ...
                switch (property) {
                    case "resourceId", "walletId" -> serviceConfiguration.put(property, value.stringValue());
                    case "maximumChargingRateWatts", "nrOfModulationSteps", "minTemperature", "maxTemperature" -> serviceConfiguration
                            .put(property, Literals.getIntValue(value, -1));
                    case "showWidget" -> serviceConfiguration.put(property, Literals.getBooleanValue(value, false));
                }
            }
    ...
    public String getConfigurationPID() {
        return "org.flexiblepower.manager.heater.HeaterManager";
    }

    @Override
    public String getBundleID() {
        return "flexiblepower.manager.heater";
    }
