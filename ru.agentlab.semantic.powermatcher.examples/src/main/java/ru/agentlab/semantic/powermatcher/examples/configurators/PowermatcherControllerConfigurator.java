...
public class PowermatcherControllerConfigurator implements ThingServiceConfigurator {
    ...
    public Dictionary<String, ?> getConfiguration(Thing thing, ConnectionContext context) {
        ...
        try (var res = conn.prepareTupleQuery(query.getQueryString()).evaluate()) {
            for (BindingSet bindings : res) {
                ...
                switch (property) {
                    case "desiredParent", "agentIdPrefix" -> serviceConfiguration.put(property, value.stringValue());
                }
            }
    ...
    public String getConfigurationPID() {
        return "net.powermatcher.fpai.controller.PowerMatcherController";
    }
